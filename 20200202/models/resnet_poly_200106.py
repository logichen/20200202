...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.bn2 = nn.BatchNorm1d(512*block.expansion, affine=False)
        self.linear1 = nn.Linear(512*block.expansion, num_classes)
        self.linear2 = nn.Linear(512*block.expansion, num_classes, bias=False)
        self.linear3 = nn.Linear(512*block.expansion, num_classes, bias=False)
        self.linear4 = nn.Linear(512*block.expansion, num_classes, bias=False)
        self.linear5 = nn.Linear(512*block.expansion, num_classes, bias=False)
        # self.linear2 = nn.Linear(512*block.expansion, num_classes, bias=False)
        # self.linear3 = nn.Linear(512*block.expansion, num_classes, bias=False)
        # self.linear4 = nn.Linear(512*block.expansion, num_classes, bias=False)


        # self.linear2 = nn.Linear(512*block.expansion, num_classes*10)
        # self.linear3 = nn.Linear(num_classes*10, num_classes)

        self.threshold = nn.Threshold(1, 0)
        self.relu1 = nn.ReLU()
        # self.logic = threshold(out + 1) - relu1(out)
    ...
    def forward(self, x):
        ...
        # # out2 = self.threshold(out + 0.5) - self.relu1(out-0.5)
        # # out2 = self.linear2(out2)
        # # out2 = self.linear3(self.threshold(out2 + 1) - self.relu1(out2))      # (torch.sign(out-0.5))# out2.shape is 256,10
        # out_binary = self.threshold(out + 0.5) - self.relu1(out-0.5)
        # # print('out_binary', out_binary)
        # out2 = self.linear2(out_binary)
        # # print('out2', out2)
        # out2 = self.threshold(out2 + 1) - self.relu1(out2)     # (torch.sign(out-0.5))# out2.shape is 256,10
        # # print('out_logic', out2)
        # out2 = self.linear3(out2)



        # for i in range(512):
        #     out1 = torch.sum(out.pow)
        # out0 = self.linear0()
        out = (out - out.min(0)[0] )/(out.max(0)[0] - out.min(0)[0])
        # print('norm_out_max', out.max())
        # print('norm_out', out)
        out1 = self.linear1(out)
        # out2 = self.linear2(out.pow(2))
        # out = out1+out2
        out2 = self.linear2(2*torch.pow(out, 2)-1)
        out3 = self.linear3(4*torch.pow(out, 3)-3*out)
        out4 = self.linear4(8*torch.pow(out, 4)-8*torch.pow(out, 2)-1)
        out5 = self.linear5(16*torch.pow(out, 5)-20*torch.pow(out, 3)+5*out)
