...
parser.add_argument('--resume', '-r', action='store_true', help='resume from checkpoint_saddle_no_dpn92')
...
start_epoch = 0  # start from epoch 0 or last checkpoint_saddle_no_dpn92 epoch
...
# net = torch.hub.load('pytorch/vision', 'mobilenet_v2', pretrained=False)

# net = ptcv_get_model("resnext101_32x48d_cifar10", pretrained=False)
...
if args.resume:
    # Load checkpoint_saddle_no_dpn92.

    print('==> Resuming from checkpoint_saddle_no_dpn92..')

    assert os.path.isdir('checkpoint_saddle_no_dpn92'), 'Error: no checkpoint_saddle_no_dpn92 directory found!'

    checkpoint_saddle_no_dpn92 = torch.load('./checkpoint_saddle_no_dpn92/ckpt_opt.pth')

    net.load_state_dict(checkpoint_saddle_no_dpn92['net'])

    best_acc = checkpoint_saddle_no_dpn92['acc']

    start_epoch = checkpoint_saddle_no_dpn92['epoch']
...
def test(epoch):
    ...
    # Save checkpoint_saddle_no_dpn92.
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint_saddle_no_dpn92'):
            os.mkdir('checkpoint_saddle_no_dpn92')

        torch.save(state, './checkpoint_saddle_no_dpn92/ckpt_opt.pth')
...
if __name__ == '__main__':
    ...
    # csvFile = open("saddle_6.csv", "w")

    # writer_csv = csv.writer(csvFile)

    # for epoch in range(start_epoch, start_epoch+200):

    train_loss_all, test_loss_all, train_acc_all, test_acc_all = torch.zeros(1), torch.zeros(1), torch.zeros(
        1), torch.zeros(1)
    ...
    if not os.path.isdir('saddle_no_dpn92'):
        os.mkdir('saddle_no_dpn92')

    csv_data.to_csv("./saddle_no_dpn92/saddle_no_dpn92.csv", mode='a+', index=None, header=None)
