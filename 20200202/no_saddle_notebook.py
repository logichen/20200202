...
parser.add_argument('--resume', '-r', action='store_true', help='resume from checkpoint_no_saddle_notebook')
...
start_epoch = 0  # start from epoch 0 or last checkpoint_no_saddle_notebook epoch
...
if args.resume:
    # Load checkpoint_no_saddle_notebook.
    print('==> Resuming from checkpoint_no_saddle_notebook..')
    assert os.path.isdir('checkpoint_no_saddle_notebook'), 'Error: no checkpoint_no_saddle_notebook directory found!'
    checkpoint_no_saddle_notebook = torch.load('./checkpoint_no_saddle_notebook/ckpt_opt.pth')
    net.load_state_dict(checkpoint_no_saddle_notebook['net'])
    best_acc = checkpoint_no_saddle_notebook['acc']
    start_epoch = checkpoint_no_saddle_notebook['epoch']
...
def test(epoch):
    ...
    # Save checkpoint_no_saddle_notebook.
    acc = 100.*correct/total
    if acc > best_acc:
        print('Saving..')
        state = {
        ...
        if not os.path.isdir('checkpoint_no_saddle_notebook'):
            os.mkdir('checkpoint_no_saddle_notebook')
        torch.save(state, './checkpoint_no_saddle_notebook/ckpt_opt.pth')
        best_acc = acc
...
if __name__ == '__main__':
    ...
    #print(test)
    if not os.path.isdir('no_saddle'):
        os.mkdir('no_saddle')
    csv_data.to_csv("./no_saddle/no_saddle.csv", mode='a+',index=None,header=None)
